...
const UnlockModal = ({ walletAddress, onClose, onSuccess }) => {
  ...
  const handleUnlock = async () => {
    ...
    try {
      ...
      alert('全球视野功能已解锁！所有案例均可查看详情和时光旅行！');
      onSuccess();
    } catch (error) {
    ...
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[60] p-4">
      <div className="bg-slate-800 rounded-2xl max-w-md w-full p-6 border border-purple-500/30">
        ...
        <ul className="text-gray-300 space-y-2 mb-6">
          <li className="flex items-start"><span className="text-green-400 mr-2">✓</span>查看全部历史投资案例</li>
          <li className="flex items-start"><span className="text-green-400 mr-2">✓</span>每个案例的财富神话故事详情</li>
          <li className="flex items-start"><span className="text-green-400 mr-2">✓</span>每个案例的时光旅行动画</li>
          <li className="flex items-start"><span className="text-green-400 mr-2">✓</span>理解AI的真正赚钱潜力</li>
        </ul>
        ...
        <div className="flex space-x-3">
          <button onClick={onClose} className="flex-1 px-4 py-3 bg-gray-700 hover:bg-gray-600 text-white font-semibold rounded-lg transition-all">
            取消
          </button>

...
## Database Schema
- `users`: `{ wallet_address, tier, balance_usd, has_global_vision, ... }`
- `bots`: `{ id, user_wallet, name, gender, avatar, level, total_profit, ... }`
- `trades`: `{ id, bot_id, symbol, action, price, amount, profit_loss, reason, ... }`
- `deposits`: `{ id, wallet_address, currency, amount, usd_value, status, ... }`
...
│   │   ├── Dashboard.js
│   │   ├── LandingPage.js
│   │   ├── GlobalVisionPage.js (with filters)

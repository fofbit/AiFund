...
async def buy_virtual_asset(req: dict):
    ...
    if bot["total_profit"] < asset_found["price"]:
        raise HTTPException(status_code=400, detail="Insufficient profit to purchase")
    
    # Check if already owned
    if asset_id in bot.get("owned_assets", []):
        raise HTTPException(status_code=400, detail="Already owned")
    
    # Purchase asset (deduct from total_profit)
    ...
    logger.info(f"{bot['name']} purchased {asset_found['name']} for ${asset_found['price']}")
